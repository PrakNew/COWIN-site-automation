...
def runthepage(driver,state,city,age,timing):
    ...
    driver.find_element_by_xpath('/html/body/app-root/div/app-home/div[2]/div/appointment-table/div/div/div/div/div/div/div/div/div/div/div[2]/form/div/div/div[2]/div/div[1]/mat-form-field/div/div[1]/div').click()
    ...
    driver.find_element_by_xpath('//*[@id="mat-select-0-panel"]').send_keys(Keys.DOWN)#selecting the dropdown and pressing the key down
    text = driver.find_element_by_xpath('//*[@id="mat-option-1"]').text#get the text for the first value in the dropdown
    i=1
    while(text.strip().lower() != state.strip()):
        text = driver.find_element_by_xpath('//*[@id="mat-option-{0}"]'.format(i)).text
        i+=1
        
    driver.find_element_by_xpath('//*[@id="mat-option-{0}"]'.format(i-1)).click()#selecting the state 
    time.sleep(1)
    ...
    driver.find_element_by_xpath('//*[@id="mat-select-2-panel"]').send_keys(Keys.DOWN)
    text1 = driver.find_element_by_xpath('//*[@id="mat-option-37"]').text
    i=37
    while(text1.strip().lower() != city.strip()):
        text1 = driver.find_element_by_xpath('//*[@id="mat-option-{0}"]'.format(i)).text
        print(text1,'--------')
        i+=1
        
    driver.find_element_by_xpath('//*[@id="mat-option-{0}"]'.format(i-1)).click()
    time.sleep(1)
    ...
    element = driver.find_element_by_xpath('/html/body/app-root/div/app-home/div[2]/div/appointment-table/div/div/div/div/div/div/div/div/div/div/div[2]/form/div/div/div[6]')
...
timing = int(input("enter the timing in seconds in which the data will be refreshed "))
